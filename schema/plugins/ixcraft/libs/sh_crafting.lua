...
-- prefix is internal (used when recursing into subfolders) - callers should never pass it themselves.
-- file.Find's glob is not recursive on its own, so without this, anything placed in a subfolder of
-- recipes/ or stations/ (e.g. recipes/ammo_recipes/sh_recipe_9mm.lua) would silently never load
function PLUGIN.craft.LoadFromDir(directory, pathType, prefix)
	prefix = prefix or ""

	for _, v in ipairs(file.Find(directory.."/sh_*.lua", "LUA")) do
		local niceName = prefix..v:sub(4, -5)
	...
	-- recurse into subfolders; each level's folder name gets baked into the uniqueID prefix so e.g.
	-- ammo_recipes/sh_recipe_9mm.lua and ammo_compact/sh_recipe_9mm.lua can never collide
	local _, folders = file.Find(directory.."/*", "LUA")

	for _, folderName in ipairs(folders) do
		PLUGIN.craft.LoadFromDir(directory.."/"..folderName, pathType, prefix..folderName.."_")
	end
end
