ITEM.name = "Cold Compress"
ITEM.description = "A cold compress to bring the swelling down."
...
ITEM.functions.UseSelf = {
    ...
    OnRun = function(item, data)
        ...
        local cured = RemoveCharacterCondition and RemoveCharacterCondition(character, "blackeye", region)

        if (cured) then
            client:Notify("The swelling goes down and your vision clears up.")
        else
            client:Notify("You don't have any swelling to treat.")
        end
...
ITEM.functions.UseOnOther = {
    ...
    OnRun = function(item, data)
        ...
        if (cured) then
            target:Notify("The swelling goes down and your vision clears up.")
            client:Notify("You treat " .. target:Name() .. "'s black eye.")
        else
            client:Notify(target:Name() .. " doesn't have any swelling to treat.")
        end
